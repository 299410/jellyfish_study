...
import { ClipboardList, Plus, Clock, FileQuestion, ChevronRight, BrainCircuit } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-lg flex items-center gap-2">
            ...
            Luyện thi trắc nghiệm thông minh với AI
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-3xl p-12 border border-slate-100 shadow-sm text-center flex flex-col items-center justify-center">
          ...
          <p className="text-slate-500 mb-6 max-w-md">
            Bạn có thể tự tạo đề thi bằng cách copy bộ câu hỏi bất kỳ trên mạng và để AI tự động phân tích thành bài kiểm tra.
          </p>
          <Link href="/quiz/import">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {quizzes.map(quiz => (
            <Link key={quiz.id} href={`/quiz/${quiz.id}/setup`}>
              <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-indigo-100/50 hover:border-indigo-200 transition-all duration-300 group cursor-pointer h-full flex flex-col">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-3 mt-4">
                    <div className="flex items-center gap-1.5 text-sm text-slate-500 bg-slate-50 px-2.5 py-1 rounded-lg">
                      ...
                      <span className="font-medium">{quiz._count.questions} câu</span>
                    </div>
                    {quiz.timeLimit && (
                      <div className="flex items-center gap-1.5 text-sm text-slate-500 bg-slate-50 px-2.5 py-1 rounded-lg">
                ...
                <div className="mt-6 pt-4 border-t border-slate-50 flex items-center justify-between text-sm">
                  <span className="text-slate-400">
                    {quiz._count.sessions} lượt làm bài
                  </span>
